                ...
                settingsSection(title: "字幕生成设置", icon: "captions.bubble", color: .appleOrange) {
                    VStack(spacing: 16) {
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Picker("", selection: Binding(
                                get: { settings.subtitleTargetLanguage },
                                set: {
                                    settings.subtitleTargetLanguage = $0
                                    autoSave()
                                }
                            )) {
                                ForEach(0..<AppConstants.subtitleTargetLanguages.count, id: \.self) { index in
                                    Text(AppConstants.subtitleTargetLanguageDisplayNames[index])
                                        .tag(AppConstants.subtitleTargetLanguages[index])
                                }
                            }
                            .pickerStyle(MenuPickerStyle())
                            .frame(maxWidth: .infinity, alignment: .leading)
