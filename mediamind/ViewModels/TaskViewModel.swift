...
enum ProcessingError: Error, LocalizedError {
    ...
    case configurationIncomplete(items: [ConfigurationCheckResult.MissingConfiguration], message: String)
    ...
    var errorDescription: String? {
        ...
        case .invalidOutputType:
            return "无效的输出类型"
        case .configurationIncomplete(_, let message):
            return message
        }
    }
    
    /// 获取缺失的配置项（仅用于 configurationIncomplete 错误）
    var missingConfigurations: [ConfigurationCheckResult.MissingConfiguration]? {
        switch self {
        case .configurationIncomplete(let items, _):
            return items
        default:
            return nil
        }
