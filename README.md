...
- **SRT 格式输出** - 自动生成标准 SRT 格式翻译字幕文件 (translation.srt)
- **多语言翻译** - 支持中文、英文、日文、韩文、法文、德文、西班牙文等 7 种目标语言
- **智能三步处理流程**：
  1. **格式转换** - 自动读取 audio.md 并转换为 SRT 时间戳格式
  2. **LLM 翻译** - 使用本地大语言模型将原始语言完整翻译为目标语言
  3. **二次校验** - 多维度检测翻译质量，自动修复未翻译或混合语言的条目
- **字幕智能切分** - 超长字幕按标点符号自动切分为多条独立条目，每条不超过 18 字，确保播放器单行显示
- **换行符清理** - 自动移除字幕文本中的换行符和多余空格，确保始终单行显示
- **增强的质量保证机制**：
  - 外语字符比例检测（阈值 15%）
  - 常用词汇模式匹配（法语/英语功能词识别）
  - 文本相似度计算（Jaccard 系数 >70% 触发重译）
  - 多轮自动重试（最多 3 次）
  - 箭头格式清理和原文残留移除
- **时间同步** - 精确的时间戳对齐，保留原始转录的时间信息
- **纯净输出** - 输出 100% 目标语言字幕，无原文残留
...
| 类型 | 说明 | 输出文件 |
|------|------|----------|
| 字幕生成（智能翻译） | LLM 翻译为目标语言的单语字幕 | translation.srt, audio.md |
...
3. **语音转录** - Whisper 转录为文本，生成 audio.md
4. **字幕翻译**（字幕模式）- LLM 智能翻译 + 自动校验修复
5. **文本分析** - LLM 内容分析（可选）
6. **报告生成** - 生成结构化报告（可选）
...
- **最后更新**: 2026-07-02
- **兼容性**: macOS 14.6+
- **最新功能**: 文档同名文件夹组织、字幕智能切分（18字限制）、提示词自定义
